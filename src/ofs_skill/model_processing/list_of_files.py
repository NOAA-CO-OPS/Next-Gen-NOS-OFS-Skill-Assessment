...
def construct_s3_url(local_path: str, prop: Any, logger: Logger) -> Optional[str]:
    ...
    try:
        ...
        if prop.ofs in ('stofs_3d_atl', 'stofs_3d_pac'):
            ...
            # STOFS uses different path structure
...
def construct_expected_files(prop: Any, dir_path: str, logger: Logger) -> list[str]:
    ...
    # Path format: .../netcdf/YYYY/MM/DD or .../netcdf/YYYYMM
    ...
    if prop.ofsfiletype == 'stations':
        ...
        # For fields files, we need to generate hourly files
        if prop.whichcast == 'nowcast':
    ...
    for date_index in range(0, dates_len):
        ...
        if not os.path.exists(model_dir) or not os.listdir(model_dir):
            ...
            if not use_s3_fallback:
                # Original behavior: try backup directory
                ...
                if not os.path.exists(model_dir):
                    logger.error(
                        'Model file path ' + model_dir + ' not found, files do '
                        'not exist. Abort!')
                    raise SystemExit(-1)
            else:
                # S3 fallback enabled: don't exit, just log and continue
                logger.info('S3 fallback enabled - will use expected directory path for URL construction')
