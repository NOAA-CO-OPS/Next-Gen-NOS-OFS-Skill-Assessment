...
from model_processing import (
    ...
    intake_scisa,
    list_of_files,
    model_format_properties,
    model_properties,
    model_source,
    write_ofs_ctlfile,
)
from obs_retrieval import format_obs_timeseries, utils
...
def get_node_ofs(prop, logger):
    ...
    for variable in ['water_level',
        ...
        try:
            ...
            filepath = os.path.join(prop.control_files_path, datum_filename)
            # Check datum report age, and overwrite only if it's > 1 hour old
            try:
                st=os.stat('C:/opt_GH/ofs_dps/control_files/cbofs_wl_datum_report.csv')
