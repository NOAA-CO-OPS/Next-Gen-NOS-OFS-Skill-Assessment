...
Below, in the nowcast section of list_of_files, the code
retrieves an extra day of output for WCOFS. This extra day helps the skill
assessment stitch together more complete nowcast  time series -- without it,
there are often time series gaps.
WCOFS time series gaps can occur for forecast output too, depending on the
start and end date of the skill assessment run. These gaps are more pronounced
for short, daily runs (<=24 hours) and are negligible for monthly or longer
runs. To avoid forecast time series gaps in daily runs, avoid start times
between 00:00 and 03:00 UTC.
...
def dates_range(start_date, end_date, ofs, whichcast):
    ...
    for i in range(
        ...
        date = datetime.strptime(start_date, "%Y%m%d%H") + timedelta(days=i)
...
def list_of_dir(prop, logger):
    ...
    for date_index in range(0, dates_len):
        ...
        # Switch to backup dir if files are not in /opt/archive/
        if not os.path.exists(model_dir) or not os.listdir(model_dir):
            logger.info("Model data path " + model_dir + " not found, or is empty. ")
            ...
            if not os.path.exists(model_dir):
                logger.error("Model data path " + model_dir + " not found. Abort!")
                sys.exit(-1)
    ...
    # Old routine -- three lines until double ##. That last ## line was already
    # commented out. Keeping it all here for reference...
    # because i'm a pack rat

    # dir_list = list(set(dir_list)) # set removes duplicates, but scrambles the order!
...
def list_of_files(prop, dir_list):
    ...
    ### PWL moved the lines below to for-loops above -- kept the code commented
    ### here just in case we need to refer back to it during testing
    # list_files[0] = [
    #     i
    #     for i in list_files[0]
    #     if int(i.split(".")[-3][-2:])
    #     >= datetime.strptime(prop.startdate, "%Y%m%d%H").day
    # ]
    # list_files[-1] = [
    #     i for i in list_files[-1] if
    #     int(i.split(".")[-3][-2:]) <= datetime.strptime(
    #         prop.enddate , "%Y%m%d%H").day]
