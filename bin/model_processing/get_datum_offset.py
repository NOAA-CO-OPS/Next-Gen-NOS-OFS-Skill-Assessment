...
def report_datums(prop, datum_offsets, logger):
    ...
    try:
        ...
        if len(datum_offsets[1]) != len(read_station_ctl_file[0]):
            logger.error("Check array dimensions in report_datums! "
                         "No report generated.")
            return

        for i in range(len(read_station_ctl_file[0])):
            if read_station_ctl_file[0][i][0] != datum_offsets[0][i]:
                logger.error("Cannot write datums report, stations are "
                             "mismatched!")
                return
            station_providers.append(read_station_ctl_file[0][i][-1])
            id_numbers.append(read_station_ctl_file[0][i][0])
            station_datums.append(read_station_ctl_file[1][i][-1])
            if (is_number(read_station_ctl_file[1][i][-3]) and
                datum_offsets[1][i] is not None):
                station_datum_offsets.append(read_station_ctl_file[1][i][-3])
                ...
                success.append('fail')
                if is_number(read_station_ctl_file[1][i][-3]):
                    station_datum_offsets.append(read_station_ctl_file[1][i][-3])
                else:
                    station_datum_offsets.append(0)
            if success[i] == 'fail':
                if read_station_ctl_file[1][i][-3] == 'RANGE':
                    reason.append('Out of geographic range (obs)')
                elif read_station_ctl_file[1][i][-3] == 'UNKNOWN':
