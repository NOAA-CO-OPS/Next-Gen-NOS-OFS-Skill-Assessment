...
Documentation for Scripts retrieve_NDBC_year_station.py

Script Name: retrieve_NDBC_year_station.py
...
different url structure than prior years.
Therefore if the final date falls within the current year we have to
iterate by month and not by year. Otherwise, data is organized into years.
...
08-01-2023    FC   Standardized all column names and units,
                                 also included WTMP in case OTMP is not
                                 found
...
def retrieve_ndbc_year_station(retrieve_input, logger):
    '''
    This function will retrieve all standard meteorological and
    oceanographic data given the station:
    ID, year, variable ['water_level', 'water_temperature', 'currents',
    'salinity', 'wind', 'air_pressure', Hs]

    In the NDBC website current year data is split by month with a
    completely different url structure than prior years.
    Therefore if the final date falls within the current year we have to
    iterate by month and not by year. Otherwise, data is organized into years.
    '''
    station = retrieve_input.station
    year = retrieve_input.year
    ...
    if variable in {'water_level', 'wind', 'air_pressure', 'Hs'}:
        url = f'{base_url}/view_text_file.php?filename=' \
              f'{str(station).lower()}h{str(year)}.txt.gz&dir=' \
              f'data/historical/stdmet/'

    elif variable in {'water_temperature', 'salinity'}:
        url = f'{base_url}/view_text_file.php?filename=' \
              f'{str(station).lower()}o{str(year)}.txt.gz&dir=' \
              f'data/historical/ocean/'
        url2 = f'{base_url}/view_text_file.php?filename=' \
               f'{str(station).lower()}h{str(year)}.txt.gz&dir=' \
               f'data/historical/stdmet/'

    elif variable == 'currents':
        url = f'{base_url}/view_text_file.php?filename=' \
              f'{str(station).lower()}a{str(year)}.txt.gz&dir=' \
              f'data/historical/adcp/'

    logger.info("Calling Retrieve NDBC Year Station service for %s", variable)
    obs = None
    obs2 = None
    if variable == 'currents':
        try:
            ...
            try:
                obs = pd.read_csv(
                    url, sep='\\s+', dtype=object).drop(
                    index=0).reset_index()
            except Exception as ex:
                logger.error(
                    'Retrieve NDBC Year Station service failed at %s '
                    '-- %s', url, str(ex))
                raise Exception from ex
    elif variable in {"water_temperature", "salinity"}:
        try:
            obs = pd.read_csv(
                url, sep="\\s+", dtype=object).drop(index=0).reset_index()
        except (ValueError, HTTPError) as ex1:
            logger.error(
                'Retrieve NDBC Year Station service failed at %s -- '
                '%s. Calling url2', url, str(ex1))
            try:
        ...
        try:
            ...
            logger.error(
                "Retrieve NDBC Year Station service failed at %s -- %s", url,
                str(ex))
            raise Exception from ex
