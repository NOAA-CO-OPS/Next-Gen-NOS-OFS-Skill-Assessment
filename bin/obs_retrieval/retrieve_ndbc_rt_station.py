...
def retrieve_ndbc_rt_station(retrieve_input, logger):
    ...
    logger.info("Calling Retrieve NDBC RT Station service: %s", url_rt)
    obs_rt = None
    obs_rt2 = None
    ...
    logger.info("Calling Retrieve NDBC RT Station service: %s", url)
    obs = None
    obs2 = None
    if variable == "currents":
        try:
    ...
    if variable == "water_level":
        ...
        if (
            ...
            try:
                ...
                logger.error(
                    "Fail to process NDBC currents data... trying realtime data... -- %s", 
                    str(ex), )
        ...
        try:
            if (
                ...
                try:
                    ...
                    logger.error(
                        "Fail to process NDBC ocean temperature data... trying realtime data... -- %s", 
                        str(ex), )

                try:
                    obs_rt.loc[(obs_rt["OTMP"] == "MM"), "OTMP"] = np.nan
                    obs_rt.loc[(obs_rt["OTMP"].astype(float) >= 98) | (
                            obs_rt["OTMP"].astype(float) < -50), "OTMP"] = np.nan
                    obs_rt.loc[(obs_rt["DEPTH"].astype(float) >= 5000) | (
                            obs_rt["DEPTH"].astype(float) < 0), "DEPTH"] = np.nan
                    obs_rt = obs_rt[["DateTime", "DEPTH", "OTMP"]]
                    obs_rt = obs_rt.dropna()
                except Exception as ex:
                    logger.error(
                        "Fail to process NDBC real time ocean temperature data -- %s", 
                        str(ex), )
            ...
            if (
                ...
                try:
                    ...
                    logger.error(
                        "Fail to process NDBC surface temperature data... trying realtime data... -- %s", 
                        str(ex), )
                ...
                try:
                    ...
                    logger.error(
                        "Fail to process NDBC real time surface temperature data -- %s", 
                        str(ex), )
        ...
        if (
            ...
            try:
                ...
                logger.error(
                    "Fail to process NDBC salinity data... trying realtime data... -- %s", 
                    str(ex), )
            try:
