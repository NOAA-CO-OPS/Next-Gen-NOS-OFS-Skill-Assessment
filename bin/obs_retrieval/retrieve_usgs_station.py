...
def retrieve_usgs_station(retrieve_input, logger):
    ...
    usgs.start = datetime.strptime(
        retrieve_input.start_date, "%Y%m%d")
    usgs.end = datetime.strptime(
    ...
    usgs.start_str = \
        f"{usgs.start_year}-{usgs.start_month}-{usgs.start_day}"
    usgs.end_str = \
        f"{usgs.end_year}-{usgs.end_month}-{usgs.end_day}"
    ...
    if retrieve_input.variable == "water_level":
        ...
        # logger.info("Calling USGS API for %s at station %s...",
        #             retrieve_input.variable, retrieve_input.station
        #             )
        for code in list_of_codes:
            ...
            try:
                with urllib.request.urlopen(usgs.url) as \
                    url_usgs:
            ...
            if (
                ...
                and len(obs["value"]["timeSeries"][0]\
                        ["values"]) > 0
            ):
                data = obs["value"]["timeSeries"][0]\
                    ["values"][0]["value"]
                date, var = [], []
        ...
        list_of_codes = ["00010",
                         ...
                         "99980",
                         "99984"]
        logger.info("Calling USGS API for %s...",
                    retrieve_input.variable
                    )
        for code in list_of_codes:
            ...
            if (
                ...
                data_length = len(data)
                for i in range(data_length):
                    fixed_date = data[i]["dateTime"].\
                        split("-")[:-1]
                    fixed_date = fixed_date[0] + \
                        fixed_date[1] + fixed_date[2]
                    date.append(fixed_date)
