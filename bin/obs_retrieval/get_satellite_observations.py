...
def list_of_urls_goes_east(hours_range1):
    """
    ...
    changedate = datetime.strptime('2025-04-07T21:00:00Z','%Y-%m-%dT%H:%M:%SZ')
...
def list_of_urls_sport(hours_range1):
    """
    This function will list the APIs for NASA SPoRT.

    The SPoRT SST Composite is blend of VIIRS, MODIS, NESDIS1, and OSTIA SST products
    (except over the inland lakes, where the NESDIS is unavailable).
    """

    url_root = "https://nssrgeo.ndc.nasa.gov/SPoRT/enhanced_sst/ghrsst_netcdf"
...
def get_sat(list_of_urls, obs2d_dir, logger, sat_type):
    ...
    for sat_dat in list_of_urls:
        ...
        if os.path.exists(sat_fname):
            ...
            if sat_type == "GOES":
                if sat_fname.find("G16") > -1 or sat_fname.find("G18") > -1 or sat_fname.find("G19") > -1:
            ...
            try:
                ...
                if "G16" in sat_dat:
                    g16date = datetime.strptime(sat_fname.split('-')[0][-14:-1],
                                                "%Y%m%d%H%M%S")
                    g16end = datetime.strptime('20250407210000',"%Y%m%d%H%M%S")
                    if g16date > g16end:
...
def get_satellite(prop, logger):
    ...
    if prop.ofs in ["ciofs",
        ...
        list_of_urls = list_of_urls_goes_east(hours)

    #Temporarily commented out 6/11/25 to avoid filling up server disk space
    list_of_urls.extend(list_of_urls_sport(hours))
    ...
    try:
        ...
        try:
            ...
            masked_sat_path = (Path(__file__).parent.parent.parent/ 
                               Path(prop.data_observations_2d_satellite_path)/
                               str(prop.ofs + '.nc')).resolve()
            ...
            if os.path.exists(masked_sat_path):
                ...
                if len(concated_sat_goes) > 0:
                    masked_sat_goes = masksat_by_ofs(concated_sat_goes, shape_file)
                ...
                if len(concated_sat_sport) > 0:
                    masked_sat_sport = masksat_by_ofs(concated_sat_sport,
                                                      shape_file)

                    masked_sat_sport.to_netcdf(
                        f"{prop.data_observations_2d_satellite_path}/{args.ofs}_sport.nc", mode="w"
                    )
