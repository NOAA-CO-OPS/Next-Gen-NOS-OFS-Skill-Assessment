...
def get_inventory_datasets(geo, t_c, usgs, ndbc, logger):
    ...
    for i in range(len(dataset_final['ID'])):
        ...
        logger.info('Station %s is %s degrees outside of the OFS shapefile.', dataset_final['ID'][i], point.distance(Polygon(geo[0])))
        if Point(dataset_final['X'][i],
                 dataset_final['Y'][i]).within(Polygon(geo[0]).buffer(0.04)) is True: # buffer to allow for PORTS stations outside of OFS shapefile
            index_true.append(i)
