...
-*- coding: utf-8 -*-
 ...
 04-17-2025    AA   Modified the scripts to loop through 1°×1° grid cells
...
def find_between(data_str, first, last):
    """
    This is a simple function to help parse the USGS data by
    selecting a string between two strings.
    """
    try:
...
def inventory_usgs_station(argu_list, start_date, end_date, logger):
    """
    This function creates an inventory of all atmospheric
    and water related usgs stations.
    To avoid selecting an unecessary number of stations,
    only those that are either realtime or have been visited
    more than 3 times are selected. This is necessary
    because there are several locations that are marked as
    usgs station but actually have no data. Create an
    inventory of USGS stations by looping through
    1°×1° grid cells.
    """
    ...
    for lat in range(math.floor(float(lat_1)),
                     math.ceil(float(lat_2))):
        lat_min = lat
        lat_max = lat + 1
        for lon in range(math.floor(float(lon_1)),
                         math.ceil(float(lon_2))):
            lon_min = lon
            lon_max = lon + 1
            ...
            try:
                ...
                logger.error("USGS data download failed at "
                             "%s -- %s.", url, str(ex))
                continue
            except Exception as ex:
                logger.error("Error during USGS inventory "
                             "data retrieval: %s", str(ex))
                continue
