...
def get_response_1(first):
    """
    Initial search for largest OFS polygon, returns min & max lat & lon
    and largest polygon
    """
    i_1, i_2, size = [], [], []
...
def get_response_2(first):
    """
    Second search for largest OFS polygon, in case first search is not
    successful. Returns min & max lat & lon and largest polygon
    """
    i_1, size = [], []
    for i in range(len(first["coordinates"])):
        i_1.append(i)
