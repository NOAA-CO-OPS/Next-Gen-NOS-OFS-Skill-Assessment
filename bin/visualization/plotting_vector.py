...
#    figheight = 900
    ...
    fig.write_html(
        r"" + f"{prop.visuals_1d_station_path}/{prop.ofs}_{station_id[0]}_"
        f"{node}_{name_var}_vector1_{naming_ws}_{prop.ofsfiletype}.html")
...
def oned_vector_plot2a(now_fores_paired, logger):
    ...
    df_obs = df_obs.groupby(['OBS_mag_binned', 'OBS_dir_binned'], observed=True)
    df_obs = df_obs.count()
    ...
    for i in range(len(now_fores_paired)):
        ...
        df_ofs.rename(
            columns={'Julian': 'freq'}, inplace=True)  # changing to freq.
        df_ofs = df_ofs.groupby(['OFS_mag_binned', 'OFS_dir_binned'], observed=True)
        df_ofs = df_ofs.count()
        ...
        maxi_r = math.ceil(
            max(
                [df_obs.groupby(['OBS_dir_binned'], as_index=False, observed=True)[
                     'percentage%'].sum().max()['percentage%'],
                df_ofs.groupby(['OFS_dir_binned'], as_index=False, observed=True)[
                     'percentage%'].sum().max()[
                     'percentage%']]) / 5) * 5
...
def oned_vector_plot2b(fig_info_data, name_var, station_id, node, prop, logger):
    ...
    fig.write_html(
        r"" + f"{prop.visuals_1d_station_path}/{prop.ofs}_{station_id[0]}_"
        f"{node}_{name_var}_vector2_{naming_ws}_{prop.ofsfiletype}.html")
...
def oned_vector_plot3(now_fores_paired, name_var, station_id, node, prop, logger):
    ...
    fig.write_html(
        r"" + f"{prop.visuals_1d_station_path}/{prop.ofs}_{station_id[0]}_"
        f"{node}_{name_var}_vector3_{naming_ws}_{prop.ofsfiletype}.html")
...
def oned_vector_diff_plot3(now_fores_paired, name_var, station_id, node, prop, logger):
    ...
    fig.write_html(
        r"" + f"{prop.visuals_1d_station_path}/{prop.ofs}_{station_id[0]}_"
        f"{node}_{name_var}_difference_vector3_{naming_ws}_{prop.ofsfiletype}.html")
