...
from visualization import create_1dplot
...
def create_1dplot_icestats(prop,time_all,logger):
    ...
    if prop.plottype == 'static':
        ...
        for i in range(len(prop.whichcasts)):
            ...
            # fig.add_trace(go.Scatter(x=dflist[i].time_all_dt,
            #                          y=dflist[i].hitrate_mod,
            #                           name='OFS ice overlap',
            #                           hovertemplate='%{y:.2f}',
            #                           legendgroup = '2',
            #                           showlegend = showlegend[i],
            #                           mode='lines',
...
def create_1dplot_ice(prop,inventory,time_all,logger):
    ...
    for i in range(0,len(inventory['ID'])):
        ...
        if prop.plottype == 'static':
            ...
            #line1 = 'Mean bias = ' + str(round(bias_value, 2))
            #line2 = 'RMSE = ' + str(round(rmse_value,2))
            #line3 = 'R = ' + str(round(r_value, 2))
            #summary = line1 + '\n' + line2 + '\n' + line3

            # Add the cool annotation to plot showing skill metrics
            # anchored_text = AnchoredText(summary, loc=3,
            ...
            create_1dplot.oned_scalar_plot(dflist,
                                            'ice_conc',
                                            inventory.at[i,'ID'],
                                            inventory.at[i,'NODE'],
                                            prop
