...
def get_title(prop, node, station_id):
    ...
    return f"<b>NOAA/NOS/CO-OPS<br>OFS Skill Assessment<br>" \
        ...
        if name_var != 'ice_conc':
            fig.add_trace(
                go.Box(
                    ...
                    #name=prop.whichcasts[i].capitalize() + " Guidance"
                    name=seriesname,
                    showlegend=False,
                    #legendgroup=prop.whichcasts[i].capitalize() + " Guidance"
                    legendgroup=seriesname,
                    width=.7,
                    line=dict(
                        color=palette[i+1],
    ...
    for i in range(len(prop.whichcasts)):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                #hovertext=list(now_fores_paired[i].BIAS_SPD),
                name=seriesname,
                #text=list(now_fores_paired[i].BIAS_DIR),
                #hovertemplate='<br>Speed Bias: %{hovertext:.2f}<br>' + 'Direction Bias: %{text:.2f}',
                ...
                #legendgroup=prop.whichcasts[i].capitalize() + " Guidance",  # + f"{i}",
                legendgroup=seriesname,
                marker=dict(
        ...
        fig.add_trace(
            go.Box(
                ...
                #name=prop.whichcasts[i].capitalize() + " Guidance",  # + f"{i}",
                name=seriesname,
                showlegend=False,
                #legendgroup=prop.whichcasts[i].capitalize() + " Guidance",  # + f"{i}",
                legendgroup=seriesname,
...
def oned_vector_plot2b(
    ...
    # palette = sns.color_palette("Spectral_r",  len(bins_mag)-1)
    ...
    fig.update_layout(
        ...
        margin=dict(l=20, r=20, t=120, b=0), legend=dict(
            # traceorder="reversed",
            font=dict(size=14, color='black', family='Arial'),
            orientation="h",  # entrywidth=100000,
...
def oned_vector_plot3(
    ...
    fig.update_layout(
        ...
        hovermode="x unified",
        #yaxis_title=f"Currents (<i>meters/second<i>)"
        #yaxis_title="V Component of Currents (<i>meters/second<i>)"
        #yaxis_title="Currents (<i>meters/second<i>)",
        #yaxis_title="",
...
def oned_vector_diff_plot3(
    ...
    for i in range(len(prop.whichcasts)):
        ...
        #u = np.array([a - b for a, b in zip(obs_u, new_u)])
        u = np.array([a - b for a, b in zip(new_u, obs_u)])
        #v = np.array([a - b for a, b in zip(obs_v, new_v)])
...
def create_1dplot(prop, logger):
    ...
    #         sys.exit(-1)
    #     else:
    #         prop.start_date_full, prop.end_date_full = get_fcst_cycle(prop,logger)
...
def oned_scalar_diff_plot(
    ...
    fig.add_trace(
        go.Scatter(
            x=list(now_fores_paired[0].DateTime),
            y=list(now_fores_paired[0].OBS), name="Observations",
            hovertemplate='%{y:.2f}', mode='lines+markers',
            line=dict(color=palette[0], width=1.5, dash = obslinestyle),
            legendgroup="obs", marker=dict(
                symbol=allmarkerstyles[0], size=8, color=palette[0],
                opacity=1, line=dict(width=0.5, color='black'))), 1, 1)
    ...
    for i in range(len(prop.whichcasts)):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                #name=prop.whichcasts[i].capitalize() + " Guidance",  # + f"{i}",
                name=seriesname,
                #hovertext=list(now_fores_paired[i].BIAS),
                #hovertemplate='<br>Bias: %{hovertext:.2f}',
                #Updated hover text to show Obs/Fore/Now values instead of bias as per user comments
                hovertemplate='%{y:.2f}',
                mode=modetype, line=dict(
                    color=palette[i+1],
