...
def list_of_json_files(filepath, prop1):
    ...
    for af_name in all_files:
        if 'model' in af_name and 'daily' not in af_name and 'SPoRT' not in af_name: #ignore daily avg
...
if __name__ == "__main__":
    ...
    try:
        for i in prop1.whichcasts:
            ...
            list_files = list_of_files.list_of_files(prop1, dir_list, logger)
            ...
            processing_2d.parse_leaflet_json(model,\
                        Path(os.path.join(prop1.path,'data','observations','2d_satellite',\
                        str(prop1.ofs+'.nc'))), prop1)
            processing_2d.parse_leaflet_json(model,\
                        Path(os.path.join(prop1.path,'data','observations','2d_satellite',\
                        str(prop1.ofs+'_sport.nc'))), prop1)
