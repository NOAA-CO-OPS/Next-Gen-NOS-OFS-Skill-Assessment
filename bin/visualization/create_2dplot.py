...
from visualization import plotting_2d, processing_2d
...
def validate_and_initialize_parameters(prop):
    ...
    try:
        check_model_files.check_model_files(prop,
                                            logger)
...
def list_of_json_files(filepath, prop1, logger):
    ...
    for af_name in all_files:
        if 'model' in af_name and 'daily' not in af_name and 'SPoRT' not in af_name and 'ssh' not in af_name: # ignore daily avg and ssh
...
if __name__ == '__main__':
    ...
    try:
        for i in prop1.whichcasts:
            ...
            dir_list = list_of_files.list_of_dir(prop1,
                                                 logger)
            list_files = list_of_files.list_of_files(prop1,
                                                 dir_list,
                                                 logger)
            logger.info('Calling intake_scisa from '
                        'create_2dplot.')
            model = intake_scisa.intake_model(list_files,
                                              prop1,
                                              logger)
