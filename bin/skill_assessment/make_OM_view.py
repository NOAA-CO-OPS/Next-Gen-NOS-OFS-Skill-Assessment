...
def make_scorecard_plot(df1, df2, prop, cast, logger):
    ...
    cminmax_rmse = [0, int(np.ceil(np.nanmax(df2.values)))]
    ...
    c_title_rmse = 'RMSE / X'
    cminmax_rmse = [0, np.ceil(np.nanmax(df2.values))]
    ...
    filename = f'scorecard_{cast}_all_OFS.html'
...
def make_skill_table(prop, cast, df, logger):
    ...
    filename = f'table_skill_{cast}_all_OFS.html'
    filepath = os.path.join(prop.om_files, filename)
    fig.write_html(filepath)
...
def make_summary_plot(prop, cast, df, logger):
    ...
    filename = f'scatter_cf_{cast}_all_OFS.html'
    filepath = os.path.join(prop.om_files, filename)
    fig.write_html(filepath)
