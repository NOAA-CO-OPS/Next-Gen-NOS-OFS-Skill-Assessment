"""
   ...
   time series and created the paired dataset (obs, and ofs) timeseries
"""
...
def paired_scalar(obs_df, ofs_df, start_date_full, end_date_full, logger):
    ...
    paired_obs = pd.concat([paired_0, obs_df]).sort_values(
        by="DateTime"
    )  # .set_index('DateTime').astype(
    # float).interpolate(method='linear').ffill().bfill().reset_index()
    ...
    if paired["OBS"].isna().all():
        #raise ValueError("All values in OBS column are NaN. Cannot proceed.")
        return None
    if paired["OFS"].isna().all():
        #raise ValueError("All values in OFS column are NaN. Cannot proceed.")
        return None
    ...
    #formatted_series = paired[[0, 1, 2, 3, 4, 5,'OBS','OFS','BIAS']].values.tolist()

    # formatted_series = []
    # for i in range(len(julian)):
    #     date_time = pd.to_datetime(paired['DateTime'])
    #     formatted_series.append(
    #         f"{float(julian[i]):13.8f} "
    #         f"{int(date_time.dt.strftime('%Y').to_numpy()[i]):4d} "
    #         f"{int(date_time.dt.strftime('%m').to_numpy()[i]):2d} "
    #         f"{int(date_time.dt.strftime('%d').to_numpy()[i]):2d} "
    #         f"{int(date_time.dt.strftime('%H').to_numpy()[i]):2d} "
    #         f"{int(date_time.dt.strftime('%M').to_numpy()[i]):2d} "
    #         f"{float(paired['OBS'].to_numpy()[i]):9.4f} "
    #         f"{float(paired['OFS'].to_numpy()[i]):9.4f} "
    #         f"{float(paired['BIAS'].to_numpy()[i]):9.4f}"
    #     )
...
def paired_vector(obs_df, ofs_df, start_date_full, end_date_full, logger):
    ...
    # Replaced concatenation with merge for paired data to ensure only matching "DateTime" entries are merged
    # while preserving all columns from `paired_ofs`.
    # The previous approach using `pd.concat()` and `groupby()` is now commented out.
    ...
    # Previous approach using concatenation and grouping (commented out):
    # paired = pd.concat([paired_obs, paired_ofs]).sort_values(by="DateTime")
    # paired = paired.groupby("DateTime", as_index=False).max
    ...
    # mask = ((paired['DateTime'] >= datetime.strptime(start_date_full,
    # '%Y%m%d-%H:%M:%S')) & (paired['DateTime'] <= datetime.strptime(
    # end_date_full, '%Y%m%d-%H:%M:%S')))
    paired = paired.loc[
        (
            (
                paired["DateTime"]
    ...
    #         f"{float(paired['OBS'].to_numpy()[i]):9.4f} "
    #         f"{float(paired['OFS'].to_numpy()[i]):9.4f} "
    #         f"{float(paired['SPD_BIAS'].to_numpy()[i]):9.4f} "
    #         f"{float(paired['OBS_DIR'].to_numpy()[i]):9.4f} "
    #         f"{float(paired['OFS_DIR'].to_numpy()[i]):9.4f} "
    #         f"{float(dir_bias[i]):9.4f}"
    #     )
