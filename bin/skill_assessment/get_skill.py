"""
 ...
 respective data and creates the paired datasets and skill table.
"""
...
from obs_retrieval import get_station_observations
from obs_retrieval import utils
from model_processing import get_node_ofs
from model_processing import model_properties
from skill_assessment import format_paired_one_d
from skill_assessment import metrics_paired_one_d
from skill_assessment import make_skill_maps
...
def ofs_ctlfile_extract(prop, name_var, logger):
    ...
    if prop.ofsfiletype == 'fields':
        ...
        if (
            ...
            get_node_ofs.get_node_ofs(prop, logger)
    ...
    logger.error(
        "%s/%s_%s_model.ctl is empty", prop.control_files_path,
        prop.ofs, name_var
    )
    return None
...
def prepare_series(read_station_ctl_file, read_ofs_ctl_file, prop,
                   name_var, i, logger):
    """
    This is creating the paired timeseries
    """
    formatted_series = "NoDataFound"
    obs_df = None
    ofs_df = None

    if read_station_ctl_file[0][i][0] == read_ofs_ctl_file[-1][i]:
        obs_path = os.path.join(prop.data_observations_1d_station_path,
                str(read_station_ctl_file[0][i][0]+"_"+prop.ofs+"_"+name_var+"_station.obs"))
        ...
        if prop.whichcast == "forecast_a":
            ...
            +"_"+prop.whichcast+"_"+str(prop.forecast_hr)+"_model.prd"
            ...
            prd_path = os.path.join(prop.data_model_1d_node_path,
                    ...
                    +"_"+str(read_ofs_ctl_file[1][i])+"_"+prop.whichcast+"_model.prd"))
            if os.path.isfile(prd_path) is False :
                logger.info(
                    "%s/%s_%s_%s_%s_%s_model.prd is missing",
                    prop.data_model_1d_node_path,
                    read_ofs_ctl_file[-1][i],
                    prop.ofs,
                    name_var,
...
def skill(read_station_ctl_file, read_ofs_ctl_file, prop, name_var, logger):
    ...
    output = {"station_id": [], "X": [], "Y": [], "node": [], "skill": []}
    ...
    for i in range(0, data_length):
        formatted_series = prepare_series(
            read_station_ctl_file, read_ofs_ctl_file, prop, name_var, i, logger
        )
        if (
            ...
            if name_var == "cu":
                ...
                logger.info("Start cu metrics for %s",
                            read_station_ctl_file[0][i][0])
                output["station_id"].append(read_station_ctl_file[0][i][0])
                output["node"].append(read_ofs_ctl_file[1][i])
                output["X"].append(read_station_ctl_file[1][i][1])
                output["Y"].append(read_station_ctl_file[1][i][0])
                output["skill"].append(
                ...
                logger.info("Start %s metrics for %s",
                            name_var, read_station_ctl_file[0][i][0])
                output["station_id"].append(read_station_ctl_file[0][i][0])
                output["node"].append(read_ofs_ctl_file[1][i])
                #temp_x = str(float(read_station_ctl_file[1][i][1])+360)
                temp_x = str(float(read_station_ctl_file[1][i][1]))
            ...
            int_path = os.path.join(prop.data_skill_1d_pair_path,
                        str(prop.ofs+"_"+name_var+"_"+read_station_ctl_file[0][i][0]
                        +"_"+str(read_ofs_ctl_file[1][i])+"_"+prop.whichcast+"_pair.int"))
            ...
            logger.info(
                "%s_%s_%s_%s_%s_pair.int is created successfully",
                prop.ofs, name_var, read_station_ctl_file[0][i][0],
                read_ofs_ctl_file[1][i], prop.whichcast)
        else:
            logger.error(
                "%s_%s_%s_%s_%s_pair.int is not created successfully",
                prop.ofs,
                name_var,
                read_station_ctl_file[0][i][0],
                read_ofs_ctl_file[1][i],
...
def get_skill(prop, logger):
    ...
    ofs_inventory(prop.ofs, Start_Date, End_Date, prop.path) and variables
...
#        prop.start_date_full = str (start_date.year) + '-' + str (
#            start_date.month) + '-' + str (start_date.day) + 'T' + str (
#            start_date.hour) + ':' + str (start_date.minute) + ':' + str (
#            start_date.second) + 'Z'
#        prop.end_date_full = str (end_date.year) + '-' + str (
#            end_date.month) + '-' + str (end_date.day) + 'T' + str (
#            end_date.hour) + ':' + str (end_date.minute) + ':' + str (
#            end_date.second) + 'Z'
    except ValueError:
        pass
    ...
    for variable in ["water_level",
        ...
        logger.info("Searching for the %s %s station ctl files",
                    prop.ofs, variable)
        ctl_path = os.path.join(prop.control_files_path,str(prop.ofs+'_'+name_var+'_station.ctl'))
        if os.path.isfile(ctl_path) is False:
            ...
            get_station_observations.get_station_observations(
...
######## Checking for the .obs files:
        for i in range(0, len(read_station_ctl_file[0])):
            ...
            if os.path.isfile(obs_path):
                ...
                logger.error(
                    "%s/%s_%s_%s_station.obs is missing... Calling Observations Module",
                    prop.data_observations_1d_station_path,
                    read_station_ctl_file[0][i][0],
                    prop.ofs,
                    name_var,
                ...
                get_station_observations.get_station_observations(
                    prop.start_date_full_before,
                    prop.end_date_full_before,
                    prop.datum,
                    prop.path,
                    prop.ofs,
                    logger
        ...
        logger.info("Searching for the %s %s ofs ctl files",
                    prop.ofs, variable)
        ...
        for i in range(0, len(read_station_ctl_file[-1])):
            if prop.whichcast == 'forecast_a':
                ...
                if os.path.isfile(
                    f"{prop.data_model_1d_node_path}/"
                    f"{read_ofs_ctl_file[-1][i]}_{prop.ofs}_{name_var}_"
                    f"{read_ofs_ctl_file[1][i]}_{prop.whichcast}_model.prd"
                ) is False :
                    logger.info(
                        "%s/%s_%s_%s_%s_%s_model.prd is missing",
                        prop.data_model_1d_node_path,
            ...
            if (
                ...
                pd.DataFrame(
                    ...
                    r"" + f"{prop.data_skill_1d_table_path}/"
                          f"skill_{prop.ofs}_"
                    f"{variable}_{prop.whichcast}.csv"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--Datum",
        required=True,
        help="prop.datum: 'MHHW', 'MHW', 'MTL', 'MSL', 'DTL', "
        "'MLW', 'MLLW', 'NAVD', 'STND'",
    )
    parser.add_argument(
        "-f",
